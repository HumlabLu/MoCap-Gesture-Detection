...
df_vels_rows = [ [0.0] * len(column_names) ] # init with zeros for timestamp 000000
row           = df_rows[0]
...
vels_filename = os.path.join( filepath_bits[0], filepath_bits[1][:-4] + "_vels.tsv" ) # Note, no error checking
print( filepath_bits, vels_filename )
...
if args.save or args.wave:
    ...
    for col in column_names[1:]:
        data = df_vels[col]
        filepath_bits  =  os.path.split( vels_filename ) # NB, now vels_filename
        if args.save:
