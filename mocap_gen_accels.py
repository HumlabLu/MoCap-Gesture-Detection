...
df_accels_rows = [ [0.0] * len(column_names) ] # init with zeros for timestamp 000000
row           = df_rows[0]
...
for ln, row in enumerate(df_rows[1:]):
    ...
    for t0,t1 in zip(triplets, prev_triplets):
        ...
        # The distances are millimeters, convert to meters, then
        # calculate speed in m/s. Freq is Hz, 200 Hz is 1/200 secs.
        new_row.append( (dist * 100.0) / freq / freq )
        ti += 1
    prev_triplets = triplets
    df_accels_rows.append( new_row )

# Distances dataframe, use original column names.
column_names = ["Timestamp"] + column_names
df_accels     = pd.DataFrame(
    df_accels_rows,
    columns=column_names
)
print( df_accels.head() )
print( df_accels.tail() )
...
accels_filename = os.path.join( filepath_bits[0], filepath_bits[1][:-4] + "_accels.tsv" ) # Note, no error checking
print( filepath_bits, accels_filename )
...
if args.save or args.wave:
    ...
    for col in column_names[1:]:
        data = df_accels[col]
        filepath_bits  =  os.path.split( accels_filename ) # NB, now accels_filename
        if args.save:
